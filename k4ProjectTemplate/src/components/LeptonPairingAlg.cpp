...
LeptonPairingAlg::~LeptonPairingAlg() {} // destructor

StatusCode LeptonPairingAlg::initialize() { return StatusCode::SUCCESS; } // initialize()
...
StatusCode LeptonPairingAlg::execute(const EventContext&) const {
  ...
  if (nLeptons > 1) {
    ...
    if (LeptonPair.size() == 2) {
      ...
      info() << "cos between leptons " << this->getCos(&(LeptonPair[0].getMomentum()), &(LeptonPair[1].getMomentum()))
             << endmsg;
...
StatusCode LeptonPairingAlg::finalize() { return StatusCode::SUCCESS; } // finalize()

float LeptonPairingAlg::dotProduct(const Vector3f* v1, const Vector3f* v2) const {
  return (*v1)[0] * (*v2)[0] + (*v1)[1] * (*v2)[1] * (*v1)[2] * (*v2)[2];
}

float LeptonPairingAlg::getMag(const Vector3f* v) const { return sqrt(this->dotProduct(v, v)); }
...
void LeptonPairingAlg::doPhotonRecovery(edm4hep::ReconstructedParticle* lepton,
  ...
  TLorentzVector* lorentzLepton =
      new TLorentzVector(lepton->getMomentum()[0], lepton->getMomentum()[1], lepton->getMomentum()[2],
                         static_cast<double>(lepton->getEnergy()));
  ...
  for (const auto pfo : *pfoCollection) {
    if (pfo.getType() == 22) {
      ...
      if (cosLeptonPhoton < cosFSRCut)
        continue;
      photons.push_back(&pfo);
      Vector3f recoLeptonMomentum = recoLepton->getMomentum();
      Vector3f momentumSum =
          Vector3f(recoLeptonMomentum[0] + photonMomentum[0], recoLeptonMomentum[1] + photonMomentum[1],
